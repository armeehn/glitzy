...
The etsch/1 sheet is the contract that matters: etsch opens these files
...
# Mirrors etsch's own presets so a sheet opens with the right defaults.
MACHINES = {
...
def export_png(h, frame=0, mm=76, dpi=300, raw=False):
    ...
    return png_bytes(im), "image/png", "glitzy-%s-f%d.png" % (h[:8], frame)
...
def export_sequence(h, mm=76, dpi=300, raw=True):
    ...
    return buf.getvalue(), "application/zip", "glitzy-%s-frames.zip" % h[:8]
...
def export_animation(h, kind="apng", fps=None):
    ...
    if kind == "gif":
        ...
        return buf.getvalue(), "image/gif", "glitzy-%s.gif" % h[:8]
    ims[0].save(buf, "PNG", save_all=True, append_images=ims[1:],
                duration=delay, loop=0, default_image=False)
    return buf.getvalue(), "image/apng", "glitzy-%s.png" % h[:8]
...
def export_sheet(items, machine="generic", dpi=300, name=None):
    """Pack kept designs onto a letter sheet as a etsch/1 project."""
    ...
    doc = {
        "name": name or ("Glitzy " + time.strftime("%Y-%m-%d")),
        "unit": "in",
...
def contact_sheet(hashes, cols=4, cell=220):
    ...
    return png_bytes(out), "image/png", "glitzy-contact.png"
