...
# proven, so a broken deploy cannot take glitzy.hq down.
set -euo pipefail
...
cp -r "$SRC/app/glitzyd" "$DEST/glitzyd"
...
if [ "${1:-}" = "--restart" ]; then
  pct exec 114 -- systemctl restart glitzyd 2>/dev/null \
    ...
    || echo "glitzyd not installed yet (run install.sh inside the container)"
fi
