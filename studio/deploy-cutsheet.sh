#!/bin/bash
# Publish Cutsheet to cutsheet.hq.ripostelabs.xyz.
#
# Cutsheet is static, so "deploying" is copying public/ into Caddy's own
# container. There is no upstream, no service to restart, and no reload needed
# for content changes -- only /etc/caddy/sites/165-cutsheet.caddy needs one.
#
# SOURCE is the tree that the tests actually ran against: LXC 111
# /root/cs-handoff, a copy of ~user/cutsheet carrying the #handoff= importer.
# When that change is merged upstream, point this at /home/user/cutsheet and
# delete the copy.
#
