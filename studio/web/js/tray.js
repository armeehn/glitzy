...
/** Pack the tray into a etsch/1 sheet and return the stored file. */
 ...
 *   Etsch, and it navigates itself.
 */
async function openInEtsch() {
  if (!S.proj.tray.length) return;
  const win = window.open('/leaving.html', '_blank');
  if (!win) {
    toast('Your browser blocked the new tab. Allow popups for Glitzy, or '
      + 'use Build .etsch.json and open it in Etsch by hand.', true);
    return;
  }
  try {
    setBusy(true, 'packing sheet', 0);
