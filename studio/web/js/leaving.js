/* The leaving-Glitzy interstitial.
 *
 ...
 * it to ?id=... when the export lands. See tray.js openInEtsch().
 *
...
async function main() {
  ...
  try {
    ...
    return fail('The Glitzy engine is not answering, so the destination '
      + 'could not be confirmed.');
  }

  const base = (health.etsch || '').replace(/\/+$/, '');
  if (!base) {
    return fail('This Glitzy has no Etsch configured '
      + '(GLITZY_ETSCH is unset), so there is nowhere to hand the '
      + 'sheet to. The file itself is fine — download it and open it in '
      + 'Etsch by hand.');
  }

  const name = q.get('name') || 'sheet.etsch.json';
  ...
  $('#lede').textContent = 'The sheet is built and waiting. Continuing opens '
    + 'Etsch in this tab and loads it there automatically.';
